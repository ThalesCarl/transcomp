...
plt.plot(x,y1,'ro', label = "Volumes finitos (resistência equivalente)")
y2=finiteVolumeDataLI.transpose()[1]

plt.plot(x,y2,'k+',markersize = 12, label = "Volumes finitos (interpolação linear)")
plt.xlabel('x [m]')
plt.ylabel('T(x) [°C]')
plt.grid(b=1)
#plt.ylim([300,370])
...
file = open("../../results/second_task/table.csv","w")
